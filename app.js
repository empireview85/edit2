            ...
            if (user) {
                window.fbDb.ref('users/' + user.uid).once('value').then(snap => {
                    ...
                    return window.fbDb.ref(DATA_ROOT).once('value').then(dataSnap => {
                        ...
                        // A bug in data processing here used to fall into the .catch() below and
                        // silently bounce the user back to the login screen with zero explanation —
                        // exactly the "why can't I log in" symptom, even though authentication itself
                        // had already succeeded. Sign-in succeeding must always be enough to get into
                        // the app; a data problem should surface loudly (the alert below), never look
                        // like a failed login.
                        if (fbData) {
                            try {
                                hotelData = fbMerge(fbData);
                                localStorage.setItem(DATA_ROOT, JSON.stringify(hotelData));
                                captureSyncSnapshot();
                            } catch (e) {
                                console.error('Failed to process hotel data on login:', e);
                                alert('Signed in, but there was a problem loading some data:\n\n' + (e.message || e) + '\n\nShowing the app anyway — please tell support what you were doing.');
                            }
                        }
                        showApp();
...
// previewDiscountIQD lets the discount input show its effect on the whole invoice/balance live,
// as the user types, without actually saving anything yet (see previewCheckoutDiscount below) —
// pass null (the default) to render from the guest's actually-saved discount.
function loadCheckOutForm(roomId, previewDiscountIQD = null) {
    const room = hotelData.rooms.find(r => r.id === roomId);
    if (!room || !room.currentGuest) return;
    const guest = hotelData.guests.find(g => g.id === room.currentGuest.id);
    if (!guest) {
    ...
    // Discount is always entered in IQD, as a flat amount off the whole stay's total (not per
    // night, not a percentage) — deducted in IQD terms regardless of whether the room itself is
    ...
    const discountIQD = previewDiscountIQD !== null ? previewDiscountIQD : (guest.discountIQD || 0);
    ...
    // Services always in IQD — computed before discount so the cascade below can spill into it.
    const serviceTotalBeforeDiscount = guest.orders.reduce((sum, order) => sum + (order.price * order.quantity), 0);

    // A discount reduces what the guest owes overall (room + services combined), not just the
    // room's own line — it first eats into the room total, and anything left over once the room
    // hits zero carries over and reduces the service total too. Without this cascade, a discount on
    // a 0-priced "Group" room with real service charges would have nothing to discount against and
    // would just be silently lost, even though the guest was told the discount covers everything.
    const discountAppliedToRoomIQD = Math.min(discountIQD, roomChargesBeforeDiscountIQD);
    const discountRemainderIQD = discountIQD - discountAppliedToRoomIQD;
    const roomChargesIQDAfterDiscount = roomChargesBeforeDiscountIQD - discountAppliedToRoomIQD;
    const roomCharges = roomIsUSD ? iqdToUSD(roomChargesIQDAfterDiscount) : roomChargesIQDAfterDiscount;
    const serviceTotal = Math.max(0, serviceTotalBeforeDiscount - discountRemainderIQD);
    ...
    content.innerHTML = `
        ...
        <div class="mb-5">
            ...
            <p class="text-xs text-gray-500 mb-2">Flat amount off the room + services total combined, always in IQD (e.g. 1,000 / 5,000 / 10,000) — never a percentage, never per night. If it's more than the room price, the rest comes off the services.</p>
            <div style="position:relative;max-width:240px;">
                ...
                <input type="text" inputmode="numeric" id="checkoutDiscountIQD"
                    ...
                    oninput="this.value=this.value.replace(/[^0-9]/g,'').replace(/\\B(?=(\\d{3})+(?!\\d))/g,',');previewCheckoutDiscount(${roomId}, this.value)"
            ...
            <table class="w-full text-sm border-collapse">
                ...
                <tbody>
                    ...
                    ${guest.orders.length > 0 ? `
                        ...
                        ${orderItems}
                        ${discountRemainderIQD > 0 ? `
                        <tr class="border-b border-gray-200">
                            <td class="py-2 text-amber-700"><i class="fas fa-percent mr-1"></i>Discount (remainder from room)</td>
                            <td class="text-right py-2 font-semibold text-amber-700">- IQD ${fmtIQD(discountRemainderIQD)}</td>
                        </tr>` : ''}
                        <tr class="bg-green-50 border-t-2 border-green-200">
                            <td class="py-2 font-bold">${t('services_label')} ${t('grand_total_label')}${discountRemainderIQD > 0 ? ' (after discount)' : ''}</td>
        ...
        <div class="flex gap-4">
            <button onclick="confirmCheckOut(${roomId}, ${roomCharges}, '${roomSymbol}', ${serviceTotal}, ${roomChargesBeforeDiscountIQD}, ${discountIQD})" class="btn btn-success flex-1" style="justify-content:center;">
...
function confirmCheckOut(roomId, roomAmount, roomSymbol, serviceAmountIQD, roomChargesBeforeDiscountIQD, discountIQD) {
    if (!requireOnline()) return;
    if (!requireActiveShift()) return;
    const room = hotelData.rooms.find(r => r.id === roomId);
    if (!room || !room.currentGuest) return;
    ...
    // Commit the discount shown on screen even if the box never lost focus (previewCheckoutDiscount
    // updates the invoice/balance live as the user types, but only saves on blur) — this button
    // always uses whatever's currently displayed, so a checkout can never go through with numbers
    // that don't match what the guest was just shown.
    if (discountIQD != null) guest.discountIQD = discountIQD;
    ...
    // "before → after" even though roomAmountPaid above is already net of the discount. Passed in
    // directly (not reconstructed as roomChargeIQD + discount) because a discount can now spill
    // over and reduce the service total too — reconstructing from the room side alone would
    // overstate "before" whenever some of the discount actually came out of services instead.
    guest.roomChargesBeforeDiscountIQD = roomChargesBeforeDiscountIQD ?? (roomChargeIQD + (guest.discountIQD || 0));
    ...
    const refundNote = (refundCashIQD > 0 || refundCashUSD > 0 || refundCardIQD > 0)
        ...
        : '';
    // Total before discount = room + services after discount, plus the discount taken back off —
    // correct whether the discount came entirely from the room or spilled into services too.
    const totalBeforeDiscountIQD = roomChargeIQD + serviceAmountIQD + (guest.discountIQD || 0);
    const discountNote = (guest.discountIQD || 0) > 0
        ? ` (discount of IQD ${fmtIQD(guest.discountIQD)} applied to the IQD ${fmtIQD(totalBeforeDiscountIQD)} total)`
        : '';
...
// `refId` is a guest id for kinds order/checkin/checkout/guestinfo, or a purchase/outsideIncome
// entry's own id for those two kinds — the two id spaces never mix per call.
function openAdminEditModal(kind, refId, orderId = null) {
    ...
    if (kind === 'order') {
        ...
        entry = (kind === 'purchase' ? (hotelData.purchases || []) : (hotelData.outsideIncome || [])).find(x => x.id === refId);
        if (!entry) return;
    ...
    document.getElementById('aeSaveBtn').onclick = () => {
        ...
        if (guest) {
            ...
            entry = (kind === 'purchase' ? (hotelData.purchases || []) : (hotelData.outsideIncome || [])).find(x => x.id === refId) || entry;
        }
...
function resetToProduction() {
    ...
    window.fbDb.ref('users').once('value').then(snap => {
        ...
        hotelData.deletedPurchaseIds = [];
        hotelData.deletedOutsideIncomeIds = [];
...
// ==================== UNASSIGNED TRANSACTIONS (ADMIN) ====================
// A record added with no shift open (the bug fixed by requireActiveShift() in v55 — some money
// actions were missing that check) has no shift window it naturally falls into, so it would
// otherwise never appear in any Shift Report, for anyone, ever, even though the money itself is
// safely saved on the guest/purchase/etc. record. This lets an admin manually hand such a record to
// a chosen staff member's chosen shift after the fact — the record's real original timestamp/staff
// are left untouched as history, only a separate assignedShiftId marker is added, which the Shift
// Report filters above (see isAssignedHere) treat as an OR-alternative to the normal match.
function isRecordCoveredByAnyShift(timestamp, staffName) {
    if (!timestamp || !staffName) return true; // nothing to check against — don't flag as orphaned
    const dt = new Date(timestamp);
    return (hotelData.shiftLog || []).some(s => {
        if (s.staff !== staffName) return false;
        const start = new Date(s.loginAt);
        const startFloor = new Date(start.getFullYear(), start.getMonth(), start.getDate(), start.getHours(), start.getMinutes(), 0, 0);
        const end = s.logoutAt ? new Date(s.logoutAt) : new Date();
        return dt >= startFloor && dt <= end;
    });
}

function findUnassignedShiftRecords() {
    const results = [];
    const roomLabelFor = g => {
        const room = (hotelData.rooms || []).find(r => r.id === g.roomId);
        return room ? room.number : (g.roomNumber || '—');
    };

    (hotelData.guests || []).forEach(g => {
        const ciTs = g.checkInRecordedAt || g.checkIn;
        if (ciTs && g.checkedInBy && !g._checkInAssignment && !isRecordCoveredByAnyShift(ciTs, g.checkedInBy)) {
            results.push({
                type: 'Check-In', label: `${g.name} — Room ${roomLabelFor(g)}`, amountText: '—',
                timestamp: ciTs, staffName: g.checkedInBy,
                getRef: () => (g._checkInAssignment = g._checkInAssignment || {})
            });
        }
        if (g.checkedOutAt && g.checkedOutBy && !g._checkOutAssignment && !isRecordCoveredByAnyShift(g.checkedOutAt, g.checkedOutBy)) {
            const amt = g.roomCurrency === 'IQD' ? `IQD ${fmtIQD(g.roomAmountPaid || 0)}` : `$${(g.roomAmountPaid || 0).toLocaleString()}`;
            results.push({
                type: 'Check-Out', label: `${g.name} — Room ${roomLabelFor(g)}`, amountText: amt,
                timestamp: g.checkedOutAt, staffName: g.checkedOutBy,
                getRef: () => (g._checkOutAssignment = g._checkOutAssignment || {})
            });
        }
        (g.orders || []).forEach(o => {
            if (o.timestamp && o.addedBy && !o.assignedShiftId && !isRecordCoveredByAnyShift(o.timestamp, o.addedBy)) {
                results.push({
                    type: 'Service', label: `${o.name} — ${g.name} (Room ${roomLabelFor(g)})`,
                    amountText: `IQD ${fmtIQD((o.price || 0) * (o.quantity || 1))}`,
                    timestamp: o.timestamp, staffName: o.addedBy, getRef: () => o
                });
            }
        });
        (g.depositLog || []).forEach(d => {
            if (d.addedAt && d.addedBy && !d.assignedShiftId && !isRecordCoveredByAnyShift(d.addedAt, d.addedBy)) {
                const parts = [d.cashIQD ? `IQD ${fmtIQD(d.cashIQD)}` : '', d.cashUSD ? `$${d.cashUSD.toFixed(2)}` : '', d.cardIQD ? `MC IQD ${fmtIQD(d.cardIQD)}` : ''].filter(Boolean);
                results.push({
                    type: 'Deposit', label: `${g.name} — Room ${roomLabelFor(g)}`,
                    amountText: parts.join(' + ') || '—', timestamp: d.addedAt, staffName: d.addedBy, getRef: () => d
                });
            }
        });
    });
    ...
    (hotelData.reservationLog || []).forEach(e => {
        if (e.reservedAt && e.reservedBy && !e.assignedShiftId && !isRecordCoveredByAnyShift(e.reservedAt, e.reservedBy)) {
            results.push({
                type: 'Reservation', label: `${e.guestName} — Room ${e.roomNumber}`,
                amountText: `IQD ${fmtIQD(e.depositIQD || 0)}`, timestamp: e.reservedAt, staffName: e.reservedBy, getRef: () => e
            });
        }
    });
    (hotelData.purchases || []).forEach(p => {
        if (p.date && p.addedBy && !p.assignedShiftId && !isRecordCoveredByAnyShift(p.date, p.addedBy)) {
            const amt = [p.priceIQD ? `IQD ${fmtIQD(p.priceIQD)}` : '', p.priceUSD ? `$${p.priceUSD.toFixed(2)}` : '', p.priceCardIQD ? `MC IQD ${fmtIQD(p.priceCardIQD)}` : ''].filter(Boolean).join(' + ');
            results.push({ type: 'Purchase', label: p.name, amountText: amt || '—', timestamp: p.date, staffName: p.addedBy, getRef: () => p });
        }
    });
    (hotelData.outsideIncome || []).forEach(p => {
        if (p.date && p.addedBy && !p.assignedShiftId && !isRecordCoveredByAnyShift(p.date, p.addedBy)) {
            const amt = [p.priceIQD ? `IQD ${fmtIQD(p.priceIQD)}` : '', p.priceUSD ? `$${p.priceUSD.toFixed(2)}` : '', p.priceCardIQD ? `MC IQD ${fmtIQD(p.priceCardIQD)}` : ''].filter(Boolean).join(' + ');
            results.push({ type: 'Outside Income', label: p.name, amountText: amt || '—', timestamp: p.date, staffName: p.addedBy, getRef: () => p });
        }
    });
...
let _unassignedRecordsCache = [];

function openReassignShiftModal() {
    if (loggedInUser?.role !== 'admin') return;
    _unassignedRecordsCache = findUnassignedShiftRecords();
    document.getElementById('reassignShiftOverlay')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'reassignShiftOverlay';
    ...
    overlay.innerHTML = `
        <div style="background:#fff;border-radius:18px;max-width:720px;width:100%;max-height:88vh;overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,0.45);display:flex;flex-direction:column;">
            <div style="background:linear-gradient(135deg,#1e3a8a,#2563eb);padding:20px 24px;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
                <div>
                    <h3 style="color:#fff;font-size:1.15rem;font-weight:800;margin:0;"><i class="fas fa-user-clock" style="margin-right:8px;"></i>Unassigned Transactions</h3>
                    <p style="color:rgba(255,255,255,0.8);font-size:0.8rem;margin:4px 0 0;">Added with no shift open — pick who it belongs to and which shift, and it'll appear in that shift's report.</p>
                </div>
                <button onclick="document.getElementById('reassignShiftOverlay').remove()" style="background:rgba(255,255,255,0.2);border:none;border-radius:8px;width:34px;height:34px;cursor:pointer;color:#fff;font-size:1rem;flex-shrink:0;"><i class="fas fa-times"></i></button>
            </div>
            <div id="reassignShiftList" style="padding:16px 20px;overflow-y:auto;flex:1;"></div>
        </div>`;
    document.body.appendChild(overlay);
    renderReassignShiftList();
}

function renderReassignShiftList() {
    const container = document.getElementById('reassignShiftList');
    if (!container) return;
    if (_unassignedRecordsCache.length === 0) {
        container.innerHTML = `<div style="text-align:center;padding:40px 20px;color:#9ca3af;"><i class="fas fa-check-circle" style="font-size:2rem;margin-bottom:10px;display:block;color:#10b981;"></i>Nothing unassigned — every record falls inside a real shift.</div>`;
        return;
    }
    const allStaffNames = [...new Set((hotelData.shiftLog || []).map(s => s.staff))].sort();
    const fmtDT = ts => new Date(ts).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    container.innerHTML = _unassignedRecordsCache.map((rec, i) => `
        <div style="border:1.5px solid #e5e7eb;border-radius:12px;padding:14px 16px;margin-bottom:12px;">
            <div style="margin-bottom:10px;">
                <span style="background:#eef2ff;color:#4338ca;font-size:0.7rem;font-weight:700;padding:3px 8px;border-radius:6px;text-transform:uppercase;letter-spacing:0.03em;">${rec.type}</span>
                <div style="font-weight:700;color:#1f2937;margin-top:6px;">${rec.label}</div>
                <div style="font-size:0.8rem;color:#6b7280;margin-top:2px;">${rec.amountText} &nbsp;·&nbsp; originally by <b>${rec.staffName}</b> &nbsp;·&nbsp; ${fmtDT(rec.timestamp)}</div>
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
                <select id="reassignStaff_${i}" onchange="populateReassignShiftOptions(${i})"
                    style="padding:7px 10px;border:2px solid #e5e7eb;border-radius:8px;font-size:0.85rem;">
                    <option value="">Select person…</option>
                    ${allStaffNames.map(n => `<option value="${n.replace(/"/g, '&quot;')}" ${n === rec.staffName ? 'selected' : ''}>${n}</option>`).join('')}
                </select>
                <select id="reassignShift_${i}" style="padding:7px 10px;border:2px solid #e5e7eb;border-radius:8px;font-size:0.85rem;min-width:220px;">
                    <option value="">Select shift…</option>
                </select>
                <button onclick="confirmReassignRecord(${i})" class="btn btn-success btn-sm"><i class="fas fa-check"></i> Assign</button>
            </div>
        </div>
    `).join('');
    // Pre-populate each row's shift dropdown to match its pre-selected staff (the person who
    // originally added it, if that name still matches a known staff member).
    _unassignedRecordsCache.forEach((rec, i) => populateReassignShiftOptions(i));
}

function populateReassignShiftOptions(i) {
    const staffSel = document.getElementById(`reassignStaff_${i}`);
    const shiftSel = document.getElementById(`reassignShift_${i}`);
    if (!staffSel || !shiftSel) return;
    const staffName = staffSel.value;
    const fmtDT = ts => new Date(ts).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    const shifts = (hotelData.shiftLog || [])
        .filter(s => s.staff === staffName)
        .sort((a, b) => new Date(b.loginAt) - new Date(a.loginAt));
    shiftSel.innerHTML = '<option value="">Select shift…</option>' + shifts.map(s =>
        `<option value="${s.id}">${fmtDT(s.loginAt)} → ${s.logoutAt ? fmtDT(s.logoutAt) : 'now (current)'}</option>`
    ).join('');
}

function confirmReassignRecord(i) {
    const rec = _unassignedRecordsCache[i];
    if (!rec) return;
    const staffSel = document.getElementById(`reassignStaff_${i}`);
    const shiftSel = document.getElementById(`reassignShift_${i}`);
    const staffName = staffSel?.value;
    const shiftId = shiftSel?.value;
    if (!staffName || !shiftId) {
        showToast('Pick both a person and a shift first.', 'error');
        return;
    }
    const shift = (hotelData.shiftLog || []).find(s => s.id === shiftId && s.staff === staffName);
    if (!shift) { showToast('Could not find that shift.', 'error'); return; }
    const ref = rec.getRef();
    ref.assignedShiftId = shiftId;
    addActivity(`Admin assigned a ${rec.type.toLowerCase()} record (${rec.label}) to ${staffName}'s shift (${new Date(shift.loginAt).toLocaleString()})`);
    showToast('Assigned to that shift.', 'success');
    _unassignedRecordsCache.splice(i, 1);
    renderReassignShiftList();
}
...
function downloadShiftReport(autoExcel = false, shiftId = null, monthKey = null, staffOverride = null) {
    ...
    // An admin can manually hand a record to a specific shift (see reassignRecordToShift) when it
    // was added with no shift open and would otherwise never appear in any report — such a record
    // carries assignedShiftId instead of matching naturally by timestamp+staff.
    const isAssignedHere = record => !!record && record.assignedShiftId === shift.id;
    ...
    const checkInsToday  = allGuests.filter(g => isAssignedHere(g._checkInAssignment)  || (inShift(g.checkInRecordedAt || g.checkIn) && byMe(g.checkedInBy)));
    const checkOutsToday = allGuests.filter(g => isAssignedHere(g._checkOutAssignment) || (inShift(g.checkedOutAt) && byMe(g.checkedOutBy)));
    const reservesToday  = resLog.filter(e => isAssignedHere(e) || (inShift(e.reservedAt) && byMe(e.reservedBy)));

    const servicesToday = [];
    allGuests.forEach(g => {
        if (!Array.isArray(g.orders)) return;
        g.orders.filter(o => isAssignedHere(o) || (inShift(o.timestamp) && byMe(o.addedBy))).forEach(o => {
    ...
    allGuests.forEach(g => {
        ...
        g.depositLog.filter(d => isAssignedHere(d) || (inShift(d.addedAt) && byMe(d.addedBy))).forEach(d => {
            const room = allRooms.find(r => r.id === g.roomId);
    ...
    const outsideIncomeToday = (hotelData.outsideIncome || []).filter(p => isAssignedHere(p) || (inShift(p.date) && byMe(p.addedBy)));
    ...
    const purchasesToday = (hotelData.purchases || []).filter(p => isAssignedHere(p) || (inShift(p.date) && byMe(p.addedBy)));
...
let _syncSnapshot = { rooms: {}, guests: {}, shiftLog: {}, reservationLog: {}, purchases: {}, outsideIncome: {} };
...
// Merges two rooms/guests arrays by `_rev` — whichever side has the higher timestamp for a given
// record wins; a record only one side has is kept unless it's in the tombstone list. Shared by
// saveDataToStorage() (merging this tab's pending write against a fresh server read) and
// applyFreshHotelData() (merging an incoming realtime update against whatever this tab currently
// has in memory) — the second use matters because without it, an update about something completely
// unrelated (e.g. another device adding a service to an occupied room) arriving while this tab's
// own save is still in flight would blindly replace the in-memory room list, visually reverting a
// change this tab just made (e.g. "Cleaning") back to the old value until the save's own write
// finally lands and the listener fires again. Busy/occupied rooms get touched more often, which is
// why this showed up worse on them than on quiet, empty rooms.
function mergeByRev(mineArr, theirsArr, tombstoneList) {
    const mineById   = new Map((mineArr   || []).map(x => [x.id, x]));
    const theirsById = new Map((theirsArr || []).map(x => [x.id, x]));
    const deletedIds = new Set((tombstoneList || []).map(t => t.id));
    const result = [];
    new Set([...mineById.keys(), ...theirsById.keys()]).forEach(id => {
        if (deletedIds.has(id)) return;
        const mine = mineById.get(id), theirs = theirsById.get(id);
...
function captureSyncSnapshotFrom(data) {
    const snap = { rooms: {}, guests: {}, shiftLog: {}, reservationLog: {}, purchases: {}, outsideIncome: {} };
    ...
    (data.shiftLog       || []).forEach(s => { const { _rev, ...rest } = s; snap.shiftLog[s.id]       = stableStringify(rest); });
    (data.reservationLog || []).forEach(e => { const { _rev, ...rest } = e; snap.reservationLog[e.id] = stableStringify(rest); });
    (data.purchases      || []).forEach(p => { const { _rev, ...rest } = p; snap.purchases[p.id]      = stableStringify(rest); });
    (data.outsideIncome  || []).forEach(p => { const { _rev, ...rest } = p; snap.outsideIncome[p.id]  = stableStringify(rest); });
    _syncSnapshot = snap;
}
...
function saveDataToStorage() {
    ...
    stampChanged(hotelData.shiftLog,       'shiftLog');
    stampChanged(hotelData.reservationLog, 'reservationLog');
    stampChanged(hotelData.purchases,      'purchases');
    stampChanged(hotelData.outsideIncome,  'outsideIncome');
    ...
    window.fbDb.ref(DATA_ROOT).once('value').then(snap => {
        ...
        // Shifts and reservations get the same protection as rooms/guests — a shift record has no
        // tombstone list (shifts are never deleted, only closed via logoutAt) and neither does a
        // reservation (cancelling one just flips its status field, it stays in the array).
        dataToSave.shiftLog       = mergeByRev(dataToSave.shiftLog,       toArr(server.shiftLog));
        dataToSave.reservationLog = mergeByRev(dataToSave.reservationLog, toArr(server.reservationLog));
        // ensurePurchaseLikeId here too — the raw server read hasn't necessarily been through
        // fbMerge's backfill yet (e.g. the very first save after this protection shipped), and both
        // sides of a merge must agree on the same id for the same legacy entry or they'd wrongly
        // look like two different records instead of one.
        dataToSave.purchases     = mergeByRev(dataToSave.purchases,     toArr(server.purchases).map(ensurePurchaseLikeId),     dataToSave.deletedPurchaseIds);
        dataToSave.outsideIncome = mergeByRev(dataToSave.outsideIncome, toArr(server.outsideIncome).map(ensurePurchaseLikeId), dataToSave.deletedOutsideIncomeIds);
...
// Re-renders the checkout invoice/balance live as the discount box is typed into, WITHOUT saving
// anything yet — bound to the input's oninput (fires on every keystroke). Without this, the Balance
// Due figure only updated on blur (the onchange-bound applyCheckoutDiscount below), which looked to
// users like "the discount doesn't apply" if they typed a number and checked the balance without
// tapping away from the field first. Refocuses the (freshly rebuilt) input afterward so typing isn't
// interrupted.
function previewCheckoutDiscount(roomId, value) {
    const discountIQD = parseFloat(String(value || '').replace(/,/g, '')) || 0;
    const input = document.getElementById('checkoutDiscountIQD');
    const hadFocus = document.activeElement === input;
    const cursorPos = hadFocus ? input.selectionStart : null;
    loadCheckOutForm(roomId, discountIQD);
    if (hadFocus) {
        const newInput = document.getElementById('checkoutDiscountIQD');
        if (newInput) {
            newInput.focus();
            const pos = Math.min(cursorPos ?? newInput.value.length, newInput.value.length);
            newInput.setSelectionRange(pos, pos);
        }
    }
}

// Flat, IQD-only discount off the stay's combined total — room + services (never per-night, never
// a percentage, and never just the room: see the cascade in loadCheckOutForm). Stored on the guest
// so it survives into History/Reports/the Activity Log as a clear before → after record.
...
function loadPurchasesPage() {
    ...
    tbody.innerHTML = display.map(({ p, i }, rowNum) => {
        ...
        return `<tr>
            ...
            ${addedByCell}
            <td>
                ${isAdmin ? `<button class="btn btn-secondary btn-sm" onclick="openAdminEditModal('purchase','${p.id}')" style="margin-right:4px;"><i class="fas fa-pen"></i></button>
...
function handlePurchaseSubmit(e) {
    ...
    hotelData.purchases.push({ id: Math.random().toString(36).substring(2, 11), name, priceIQD, priceUSD, priceCardIQD, notes, date: new Date().toISOString(), addedBy: loggedInUser?.name || '—' });
    saveDataToStorage();
...
function deletePurchase(id) {
    if (!requireOnline()) return;
    const idx = (hotelData.purchases || []).findIndex(p => p.id === id);
    if (idx === -1) return;
    hotelData.purchases.splice(idx, 1);
    // Record a tombstone so a merge against a device that hasn't heard about this delete yet doesn't
    // resurrect it — same mechanism already used for deleted rooms/guests.
    recordTombstone('deletedPurchaseIds', id);
    saveDataToStorage();
...
function loadOutsideIncomePage() {
    ...
    tbody.innerHTML = display.map(({ p, i }, rowNum) => {
        ...
        return `<tr>
            ...
            ${addedByCell}
            <td>
                ${isAdmin ? `<button class="btn btn-secondary btn-sm" onclick="openAdminEditModal('outsideIncome','${p.id}')" style="margin-right:4px;"><i class="fas fa-pen"></i></button>
                <button class="btn btn-danger btn-sm" onclick="deleteOutsideIncome('${p.id}')"><i class="fas fa-trash"></i></button>` : ''}
            </td>
        </tr>`;
    }).join('');
}
...
function handleOutsideIncomeSubmit(e) {
    ...
    hotelData.outsideIncome.push({ id: Math.random().toString(36).substring(2, 11), name, priceIQD, priceUSD, priceCardIQD, notes, date: new Date().toISOString(), addedBy: loggedInUser?.name || '—' });
    saveDataToStorage();
...
function deleteOutsideIncome(id) {
    if (!requireOnline()) return;
    const idx = (hotelData.outsideIncome || []).findIndex(p => p.id === id);
    if (idx === -1) return;
    hotelData.outsideIncome.splice(idx, 1);
    recordTombstone('deletedOutsideIncomeIds', id);
    saveDataToStorage();
...
// Purchases/outside-income entries had no unique id before v58 (they were only ever referred to by
// their position in the list), so old entries synced from Firebase won't have one. Backfill a
// stable one derived from content that can't change (date + name + who added it + amounts) — using
// a deterministic hash rather than a random id means that if two devices each backfill the very same
// legacy entry independently before either has synced, they land on the identical id instead of two
// different ones, so the record can't appear to "split in two" once merged.
function stableHashId(str) {
    let h = 5381;
    for (let i = 0; i < str.length; i++) h = ((h << 5) + h + str.charCodeAt(i)) | 0;
    return 'legacy_' + (h >>> 0).toString(36);
}
function ensurePurchaseLikeId(p) {
    if (!p || p.id) return p;
    return { ...p, id: stableHashId(`${p.date}|${p.name}|${p.addedBy}|${p.priceIQD || 0}|${p.priceUSD || 0}|${p.priceCardIQD || 0}`) };
}
...
function fbMerge(fbData) {
    ...
    return {
        ...
        purchases:      toArr(fbData.purchases).map(ensurePurchaseLikeId),
        outsideIncome:       toArr(fbData.outsideIncome).map(ensurePurchaseLikeId),
        ...
        deletedPurchaseIds:      toArr(fbData.deletedPurchaseIds),
        deletedOutsideIncomeIds: toArr(fbData.deletedOutsideIncomeIds),
...
function applyFreshHotelData(data) {
    if (!data) return;
    try {
        const fresh = fbMerge(data);
        // Merge by _rev against what's currently in memory rather than blindly replacing it — a
        // change this tab just made locally (e.g. clicking "Cleaning") already has a fresh _rev
        // stamped on it synchronously, before this listener callback can ever run, so it correctly
        // wins here even if the incoming update is about something else and doesn't reflect it yet.
        fresh.rooms  = mergeByRev(hotelData.rooms,  fresh.rooms,  fresh.deletedRoomIds);
        fresh.guests = mergeByRev(hotelData.guests, fresh.guests, fresh.deletedGuestIds);
        fresh.shiftLog       = mergeByRev(hotelData.shiftLog,       fresh.shiftLog);
        fresh.reservationLog = mergeByRev(hotelData.reservationLog, fresh.reservationLog);
        fresh.purchases      = mergeByRev(hotelData.purchases,      fresh.purchases,      fresh.deletedPurchaseIds);
        fresh.outsideIncome  = mergeByRev(hotelData.outsideIncome,  fresh.outsideIncome,  fresh.deletedOutsideIncomeIds);
        hotelData = fresh;
        localStorage.setItem(DATA_ROOT, JSON.stringify(hotelData));
        captureSyncSnapshot();
        updateNotificationBell();
